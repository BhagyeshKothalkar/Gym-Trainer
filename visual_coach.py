...
class VisualChain:
    def __init__(self):
        ...
        # 4. Summary Prompt (The Head Coach)
        self.summary_prompt = ChatPromptTemplate.from_template(
            """
            You are a Lead Sports Scientist. created a final session report.
            
            INPUT DATA:
            A list of frame-by-frame observations from an exercise session:
            {observations}
            
            TASK:
            Synthesize these individual observations into a cohesive "Engineering Corrections" report.
            Identify the top 3 most critical biomechanical patterns that need fixing.
            
            OUTPUT FORMAT (JSON ONLY):
            {{
                "feedback_summary": "A 2-sentence high-level summary of the entire session performance.",
                "corrections": [
                    {{
                        "title": "Short, Punchy Title (e.g., SPINAL ALIGNMENT)",
                        "description": "Precise technical explanation of the issue."
                    }},
                    {{
                        "title": "Title 2",
    ...
    def analyze_images(self, user_img_b64, trainer_img_b64, exercise_name="Exercise"):
        ...
        try:
            ...
            return "Could not generate feedback.", "Error"

    def generate_session_summary(self, observations):
        """
        Synthesizes multiple observations into a final JSON report.
        """
